...
const findUserByEmail = async (email) => {
  const query = 'SELECT id, name, email, password_hash, department_name, role, created_at FROM users WHERE email = $1'
  const { rows } = await pool.query(query, [email.toLowerCase()])
  return rows[0]
}

const findUserById = async (id) => {
  const query = 'SELECT id, name, email, department_name, role, created_at FROM users WHERE id = $1'
...
const createGoogleUser = async ({ name, email, department_name, role = 'user' }) => {
